 ...
 * @Last Modified time: 2018-06-16 00:30:45
 */
...
void pfOptimizer::prepOpt() {
  ...
  if (atag == "LD_MMA")
    nalg = nlopt::LD_MMA;
  else if (atag == "LD_LBFGS")
    nalg = nlopt::LD_LBFGS;
  else if (atag == "GN_ESCH")
    nalg = nlopt::GN_ESCH;
  else if (atag == "GN_ISRES")
    nalg = nlopt::GN_ISRES;
  else if (atag == "LN_SBPLX")
    nalg = nlopt::LN_SBPLX;
  else
...
double pfOptimizer::objectiveFuc(const vector<double> &x,
                                 vector<double> &grad) {
  if (gtag)
    return pfhm->errFunctGrad(x, grad);
  else
    return pfhm->errFunct(x);
}
