...
void PairMEAMC::compute(int eflag, int vflag) {
  ...
  if (eflag || vflag)
    ...
    evflag = vflag_fdotr = eflag_global = vflag_global = eflag_atom =
        vflag_atom = 0;
  ...
  FILE *fid = fopen("lmp.txt", "w");

  for (ii = 0; ii < inum_half; ii++) {
    i = ilist_half[ii];
    meam_inst->meam_dens_init(i, ntype, type, map, x, numneigh_half[i],
                              firstneigh_half[i], numneigh_full[i],
                              firstneigh_full[i], offset);
  ...
  for (ii = 0; ii < inum_half; ii++) {
    i = ilist_half[ii];
    meam_inst->meam_force(
        i, eflag_either, eflag_global, eflag_atom, vflag_atom, &eng_vdwl, eatom,
        ntype, type, map, x, numneigh_half[i], firstneigh_half[i],
        numneigh_full[i], firstneigh_full[i], offset, f, vptr);
    offset += numneigh_half[i];
  }
  ...
  for (int it = 0; it < 200; it++) {
    ...
    i = ilist_half[it];
    for (int jn = 0; jn < numneigh_half[i]; jn++) {
      int jj = listhalf->firstneigh[i][jn];
      vector<double> tm(6);
      tm[0] = x[jj][0];
      tm[1] = x[jj][1];
      tm[2] = x[jj][2];
      tm[3] = meam_inst->dscrfcn[offset + jn];
      tm[4] = meam_inst->scrfcn[offset + jn];
      tm[5] = meam_inst->fcpair[offset + jn];
      vc.push_back(tm);
    }
    sort(vc.begin(), vc.end());
    for (int it = 0; it < vc.size(); it++)
      fprintf(fid, "%.4f %.4f %.4f %.4f %.4f %.4f\n", vc[it][0], vc[it][1],
              vc[it][2], vc[it][3], vc[it][4], vc[it][5]);
    vector<double> tm(7);
    tm[0] = x[i][0];
    tm[1] = x[i][1];
    tm[2] = x[i][2];
...
double PairMEAMC::init_one(int i, int j) { return cutmax; }
...
void PairMEAMC::read_files(char *globalfile, char *userfile) {
  ...
  while (1) {
    ...
    if (strcmp(words[1], "fcc") == 0)
      lat[i] = FCC;
    else if (strcmp(words[1], "bcc") == 0)
      lat[i] = BCC;
    else if (strcmp(words[1], "hcp") == 0)
      lat[i] = HCP;
    else if (strcmp(words[1], "dim") == 0)
      lat[i] = DIM;
    else if (strcmp(words[1], "dia") == 0)
      lat[i] = DIA;
    else
  ...
  meam_inst->meam_setup_global(nelements, lat, z, ielement, atwt, alpha, b0, b1,
                               b2, b3, alat, esub, asub, t0, t1, t2, t3, rozero,
                               ibar);
  ...
  while (1) {
    ...
    if (which == 4) {
      if (strcmp(params[nparams - 1], "fcc") == 0)
        value = FCC;
      else if (strcmp(params[nparams - 1], "bcc") == 0)
        value = BCC;
      else if (strcmp(params[nparams - 1], "hcp") == 0)
        value = HCP;
      else if (strcmp(params[nparams - 1], "dim") == 0)
        value = DIM;
      else if (strcmp(params[nparams - 1], "dia") == 0)
        value = DIA;
      else if (strcmp(params[nparams - 1], "b1") == 0)
        value = B1;
      else if (strcmp(params[nparams - 1], "c11") == 0)
        value = C11;
      else if (strcmp(params[nparams - 1], "l12") == 0)
        value = L12;
      else if (strcmp(params[nparams - 1], "b2") == 0)
        value = B2;
      else
...
int PairMEAMC::pack_forward_comm(int n, int *list, double *buf, int pbc_flag,
                                 int *pbc) {
  int i, j, k, m;
...
void PairMEAMC::neigh_strip(int inum, int *ilist, int *numneigh,
                            int **firstneigh) {
  int i, j, ii, jnum;
  int *jlist;
