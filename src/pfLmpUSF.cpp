 ...
 * @Last Modified time: 2018-02-04 17:32:31
 */
...
void lmpDriver::calGSFenergy() {
  ...
  for (int iter = 0; iter < samplePoints; iter++) {
    ...
    if (myRank == 0) {
      ...
      sprintf(lmpcmds[i++],
              "region	whole  block  0  ${widthx}  0  ${widthy}  0  ${ztotal} "
              " "
              "units box");
      sprintf(lmpcmds[i++], "create_box  1   whole");
      sprintf(lmpcmds[i++],
              "lattice   bcc  ${a} orient x -1 1 1 orient y 1 -1  2 orient  z "
              "1 1 0");
      sprintf(lmpcmds[i++],
              "region	mybody  block  0  ${widthx}  0  ${widthy}  0  ${zhigh} "
              " "
              "units box");
      sprintf(lmpcmds[i++], "create_atoms   1  region   mybody");
      sprintf(lmpcmds[i++],
              "region 	 top   block   0  ${widthx}  0  ${widthy}  ${zlow}  "
              "${zhigh}  units box");
      ...
      // sprintf(lmpcmds[i++], "dump  1  all   cfg   500000  cfg/Nb_*_%d.cfg
      // mass  type xs ys zs", iter);
      // --------------------- FORCE FIELDS ---------------------
      sprintf(lmpcmds[i++],
              "displace_atoms  gtop    move   ${x_displace}  0  0  units box");
    ...
    if (myRank == LmpRoot) {
      ...
      if (iter == 0) {
        ...
        if (mycalOpt.usstag == 1) {
          ...
          /*** use tuned function ***/
          ussfErr[0] =
              (ussf_weight * targetFunct(ustacking_fault_110, ussf110));
          errUssf += ussfErr[0];
        }
      }
    }
    ...
    if (myRank == LmpRoot) {
      ...
      if (iter == 0) {
        ...
        if (mycalOpt.usstag == 1) {
          ...
          // ussf211.targ_x);
          ussfErr[1] =
              (ussf_weight * targetFunct(ustacking_fault_211, ussf211));
          errUssf += ussfErr[1];
        }
      }
    }
